...
# Yapılandırma parser objesini yarat
config = configparser.ConfigParser()

# settings.cfg dosyasını oku
config.read('settings.cfg')

# Ayarları değişkenlere ata
openai_apikey = config['option']['openai-apikey']
openai_proxy = config['option']['openai-proxy']
prompt = config['option']['prompt']
bilingual_output = config.getboolean('option', 'bilingual-output')
langcode = config['option']['langcode']
startpage = config.getint('option', 'startpage')
endpage = config.getint('option', 'endpage')
transliteration_list = config['option']['transliteration-list']
case_matching = config.getboolean('option', 'case-matching')
deepl_auth_key = config['option']['deepl-auth-key']
api_endpoint = config['option']['api-endpoint']
translation_service = config['option']['translation-service']
...
def translate_with_deepl(text_to_translate, target_language_code, deepl_auth_key, api_endpoint):
    headers = {
        'Authorization': f'DeepL-Auth-Key {deepl_auth_key}',
        'Content-Type': 'application/json'
    }
    payload = {
        'text': [text_to_translate],  # text_to_translate tek bir string ise, listeye çevrilir.
        'target_lang': target_language_code
    }
    response = requests.post(api_endpoint, headers=headers, json=payload)
    
    if response.status_code == 200:
        translated_text_payload = response.json()
        if 'translations' in translated_text_payload and len(translated_text_payload['translations']) > 0:
            translated_text = translated_text_payload['translations'][0]['text']
            return translated_text
        else:
            print("Çeviri yapılırken bir hata oluştu, cevap düzgün formatta değil.")
            return None
    else:
        print(f"DeepL API hatası: HTTP {response.status_code} - {response.text}")
        return None
    
def translate_text(text, translation_service):
    print(f"Çeviri hizmeti: {translation_service}")
    global cost_tokens
    if translation_service == 'deepl':
        return translate_with_deepl(text, 'TR', deepl_auth_key, api_endpoint)
...
def translate_and_store(text, translation_service):
    global translated_dict, cost_tokens

    if text in translated_dict:
        return translated_dict[text]

    translated_text = translate_text(text, translation_service)
    if translated_text is None:
        print("Çeviri başarısız. Metin çevrilemedi.")
        return None  # Fonksiyon burada None döndürüyor olabilir
...
if filename.endswith('.epub'):
    ...
    for item in tqdm(items):
        ...
        if item.get_type() == ebooklib.ITEM_DOCUMENT:
            ...
            for short_text in tqdm(short_text_list):
                ...
                count += 1
                # 翻译当前短文本
                translated_short_text = translate_and_store(text, translation_service)
   ...
   # 将epub书籍写入文件
    ...
    for short_text in tqdm(short_text_list):
        print(return_text(short_text))
        # 翻译当前短文本
        translated_short_text = translate_and_store(short_text, translation_service)
        short_text = return_text(short_text)
        translated_short_text = return_text(translated_short_text)
        # 将当前短文本和翻译后的文本加入总文本中
        if bilingual_output.lower() == 'true':
...
# Diğer tüm importlar, fonksiyonlar vb. tanımlarınız...

if __name__ == "__main__":
    # ... Diğer kodlarınız ...

    # Dosyanın tipine göre çeviri yapılır.
    if filename.lower().endswith('.pdf'):
        text = convert_pdf_to_text(filename, startpage, endpage)
        short_text_list = split_text(text)
        for short_text in short_text_list:
            translated_short_text = translate_and_store(short_text, translation_service)
            if translated_short_text is not None:  # translated_short_text None değilse devam edilir.
                processed_text = return_text(translated_short_text)
                # processed_text ile ilgili işlemler burada yapılır.
            else:
                print(f"Metni çevirme başarısız oldu: {short_text}")
                # Bu metin parçası atlanır ve döngü devam eder.